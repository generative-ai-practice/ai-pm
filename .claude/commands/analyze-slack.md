...
## Step 2: Slackログ分析 → Issue候補抽出（スライディングウィンドウ方式）

Slackログファイルはサイズが大きいため、**スライディングウィンドウ方式**で少しずつ読み込みながら分析する。

### 読み込み開始位置の決定

対象月ごとに読み込み開始位置を決定する:
- **開始月のログ**（例: 開始日が `2026-01-31` なら `output/slack-2026-01.md`）:
  Grepツールで `[{開始日}` のパターンを検索し、最初にヒットした行番号から読み込みを開始する。
  開始日より前のメッセージは読む必要がない。
- **2ヶ月目以降のログ**: ファイル先頭から読み込む。

### スライディングウィンドウ読み込み

以下の手順で、ログ全体をウィンドウ単位で読み込み・分析する:

1. **ウィンドウサイズ**: 100行
2. **スライド幅**: 50行（前後50行のオーバーラップを持たせ、文脈の断絶を防ぐ）
3. 手順:
   - 開始位置から100行を読み込み（Readツールで `offset` と `limit` を指定）
   - そのウィンドウ内のメッセージを分析し、Issue候補を抽出する
   - 50行進めて次の100行を読み込む
   - ファイル末尾に達するまで繰り返す
...
全ウィンドウの分析が完了したら、抽出されたトピックをマージする:
- 同一トピック（同じ会話や同じ話題）は1つにまとめる
- 複数ウィンドウにまたがるトピックは、関連するタイムスタンプを統合する
- 最終的に重複のないトピック一覧を作成する
